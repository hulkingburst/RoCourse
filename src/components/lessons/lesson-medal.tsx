 ...
 * lesson's quick checks were solved on the first pick. Reads the progress store
 * by lesson slug, so it can live in server-rendered layouts.
 */
export function LessonMedalBadge({ slug, quizCount, className }: LessonMedalProps) {
  const record = useProgressStore((state) => state.lessons[slug]);
  const firstTry = record?.firstTrySolvedSteps?.length ?? 0;
  const { medal, correct, total } = lessonMedal(firstTry, quizCount);
  if (!medal) return null;

  return (
    <span
      title={`${medalLabel(medal)} medal — ${Math.min(correct, total)} of ${total} quick checks on the first try`}
